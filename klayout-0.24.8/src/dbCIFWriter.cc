...
void 
CIFWriter::write (const db::Layout &layout, tl::OutputStream &stream, const db::SaveLayoutOptions &options)
{
  const CIFWriterOptions *cif_options = dynamic_cast <const CIFWriterOptions *> (options.get_specific_options ());
  if (cif_options) {
    m_options = *cif_options;
  } else {
    m_options = CIFWriterOptions ();
  }
...
CIFWriter::write_texts (const db::Layout &layout, const db::Cell &cell, unsigned int layer, double sf)
{
  ...
  while (! shape.at_end ()) {
    ...
    *this << "94 " << tl::to_word_or_quoted_string(shape->text_string(), "0123456789:<>/&%$!.-_#+*?\\[]{}");
    ...
    db::Point p (shape->text_trans ().disp () * sf);
