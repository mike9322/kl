    ...
    for (std::vector<db::PCellParameterDeclaration>::const_iterator p = header->declaration ()->parameter_declarations ().begin (); p != header->declaration ()->parameter_declarations ().end (); ++p, ++i) {
      ...
      if (i < m_parameters.size () && p->get_type () == db::PCellParameterDeclaration::t_shape && ! p->is_hidden ()) {
        ...
        if (m_parameters[i].is_user<db::DBox> ()) {
          ...
          shapes (layout ()->guiding_shape_layer ()).insert (db::BoxWithProperties(db::Box::from_double (m_parameters[i].to_user<db::DBox> () * (1.0 / layout ()->dbu ())), layout ()->properties_repository ().properties_id (props)));

        } else if (m_parameters[i].is_user<db::DEdge> ()) {

          shapes (layout ()->guiding_shape_layer ()).insert (db::EdgeWithProperties(db::Edge::from_double (m_parameters[i].to_user<db::DEdge> () * (1.0 / layout ()->dbu ())), layout ()->properties_repository ().properties_id (props)));

        } else if (m_parameters[i].is_user<db::DPoint> ()) {

          db::DPoint p = m_parameters[i].to_user<db::DPoint> ();
          shapes (layout ()->guiding_shape_layer ()).insert (db::BoxWithProperties(db::Box::from_double (db::DBox (p, p) * (1.0 / layout ()->dbu ())), layout ()->properties_repository ().properties_id (props)));
