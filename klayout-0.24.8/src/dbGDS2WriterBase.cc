...
void
GDS2WriterBase::write (const db::Layout &layout, tl::OutputStream &stream, const db::SaveLayoutOptions &options)
{
  set_stream (stream);
  ...
  db::GDS2WriterOptions gds2_default_options;
  const db::GDS2WriterOptions *gds2_options = dynamic_cast<const db::GDS2WriterOptions *> (options.get_specific_options ());
  if (! gds2_options) {
    gds2_options = &gds2_default_options;
  }

  add_meta_info (MetaInfo ("dbuu", tl::to_string (QObject::tr ("Database unit in user units")), tl::to_string (dbu / std::max (1e-9, gds2_options->user_units))));
  add_meta_info (MetaInfo ("dbum", tl::to_string (QObject::tr ("Database unit in meter")), tl::to_string (dbu * 1e-6)));
  add_meta_info (MetaInfo ("libname", tl::to_string (QObject::tr ("Library name")), gds2_options->libname));
  ...
  add_meta_info (MetaInfo ("mod_time", tl::to_string (QObject::tr ("Modification Time")), str_time));
  add_meta_info (MetaInfo ("access_time", tl::to_string (QObject::tr ("Access Time")), str_time));
        ...
        for (int i = 0; i < 2; ++i) {
          ...
          if (n == 0) {
            *q = db::Point ();
            ...
            if (q->x () < 0) {
              p += db::Point ((n - 1) * q->x (), 0);
              q->set_x (-q->x ());
            } 
            if (q->y () < 0) {
              p += db::Point (0, (n - 1) * q->y ());
              q->set_y (-q->y ());
            } 
          }
