...
fill_region (db::Cell *cell, const db::Polygon &fp0, db::cell_index_type fill_cell_index, const db::Box &fc_bbox, const db::Point &origin, bool enhanced_fill, 
             std::vector <db::Polygon> *remaining_parts, const db::Point &fill_margin)
{
      ...
      for (size_t i = 0; i < nx; ++i) {
        ...
        for (size_t j = 0; j < ny; ) {
          ...
          if (am.get (i, j) >= amax) {
            ...
            db::Point p0 (am.p0 () - fc_bbox.p1 ());
            p0 += db::Point (db::Coord (i) * fc_bbox.width (), db::Coord (j) * fc_bbox.height ());

            db::CellInstArray array;

            if (jj > j + 1) {
              array = db::CellInstArray (db::CellInst (fill_cell_index), db::Trans (p0), db::Point (0, fc_bbox.height ()), db::Point (fc_bbox.width (), 0), (unsigned long) (jj - j), 1);
            } else {
