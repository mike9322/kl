...
{
  return tl::Variant (1, new QImage (img), &qimage_class_instance);
}
...
MouseTrackerWidget::MouseTrackerWidget (QWidget *parent)
  : QWidget (parent, 
    ...
    m_basic_pm (QString::fromAscii (":/gtf_basic.png")),
    m_lb_pm (QString::fromAscii (":/gtf_lb.png")),
    m_mb_pm (QString::fromAscii (":/gtf_mb.png")),
    m_rb_pm (QString::fromAscii (":/gtf_rb.png"))
{
...
write_data (const tl::Variant &data, std::ostream &os, unsigned int level)
{
  if (data.is_list ()) {
    ...
    } else if (data.is_user () && data.user_type () == 1) {
...
Recorder::Recorder (QObject *parent, const std::string &log_file)
  : m_recording (false), m_save_incremental (false), m_log_file (log_file)
{
...
probe_widget (QTextEdit *te)
{
  ...
  QStringList lines = te->toPlainText ().split (QString::fromAscii ("\n"));
...
GtfXmlHandler::endElement (const QString &namespaceURI, const QString &localName, const QString &qName)
{
  ...
  } else if (localName == QString::fromAscii ("img")) {

    QByteArray ba (QByteArray::fromBase64 (m_cdata.toAscii ()));
    QImage img;
    img.loadFromData (ba);
    tl_assert (m_data_stack.size () >= 1);
    m_data_stack.end ()[-1].push (image_to_variant (img));

  } else if (localName == QString::fromAscii ("mouse_button_release") || 
             localName == QString::fromAscii ("mouse_button_press") ||
             localName == QString::fromAscii ("mouse_button_dbl_click") ||
             localName == QString::fromAscii ("mouse_move") ||
             localName == QString::fromAscii ("key_press") ||
             localName == QString::fromAscii ("key_release") ||
             localName == QString::fromAscii ("action") ||
             localName == QString::fromAscii ("resize") ||
             localName == QString::fromAscii ("probe") ||
             localName == QString::fromAscii ("error") ||
             localName == QString::fromAscii ("block")) {
    leave_event ();
  }
...
{
  if (localName == QString::fromAscii ("mouse_button_release") || 
      localName == QString::fromAscii ("mouse_button_press") ||
      localName == QString::fromAscii ("mouse_button_dbl_click")) {

    int xpos = atts.value (QString::fromAscii ("xpos")).toInt ();
    int ypos = atts.value (QString::fromAscii ("ypos")).toInt ();
    int button = atts.value (QString::fromAscii ("button")).toInt (0, 16);
    int modifiers = atts.value (QString::fromAscii ("modifiers")).toInt (0, 16);

    QEvent::Type type;
    if (localName == QString::fromAscii ("mouse_button_release")) {
      type = QEvent::MouseButtonRelease;
    } else if (localName == QString::fromAscii ("mouse_button_press")) {
    ...
    enter_event (new LogMouseEvent (tl::to_string (atts.value (QString::fromAscii ("target"))), mouse_event, mp_locator->lineNumber ()));

  } else if (localName == QString::fromAscii ("mouse_move")) {

    int xpos = atts.value (QString::fromAscii ("xpos")).toInt ();
    int ypos = atts.value (QString::fromAscii ("ypos")).toInt ();
    int buttons = atts.value (QString::fromAscii ("buttons")).toInt (0, 16);
    int modifiers = atts.value (QString::fromAscii ("modifiers")).toInt (0, 16);
  ...
  } else if (localName == QString::fromAscii ("key_press") ||
             localName == QString::fromAscii ("key_release")) {

    int key = atts.value (QString::fromAscii ("key")).toInt (0, 16);
    QChar text_char = QChar (atts.value (QString::fromAscii ("code")).toInt (0, 16));
    QString text = text_char;
    int modifiers = atts.value (QString::fromAscii ("modifiers")).toInt (0, 16);

    QEvent::Type type;
    if (localName == QString::fromAscii ("key_press")) {
    ...
    enter_event (new LogKeyEvent (tl::to_string (atts.value (QString::fromAscii ("target"))), key_event, mp_locator->lineNumber ()));

  } else if (localName == QString::fromAscii ("action")) {

    enter_event (new LogActionEvent (tl::to_string (atts.value (QString::fromAscii ("target"))), tl::to_string (atts.value (QString::fromAscii ("action"))), mp_locator->lineNumber ()));

  } else if (localName == QString::fromAscii ("resize")) {

    int xsize = atts.value (QString::fromAscii ("xsize")).toInt ();
    int ysize = atts.value (QString::fromAscii ("ysize")).toInt ();
    int xsize_old = atts.value (QString::fromAscii ("xsize_old")).toInt ();
    int ysize_old = atts.value (QString::fromAscii ("ysize_old")).toInt ();

    enter_event (new LogResizeEvent (tl::to_string (atts.value (QString::fromAscii ("target"))), QSize (xsize, ysize), QSize (xsize_old, ysize_old), mp_locator->lineNumber ()));

  } else if (localName == QString::fromAscii ("probe")) {

    enter_event (new LogProbeEvent (tl::to_string (atts.value (QString::fromAscii ("target"))), mp_locator->lineNumber ()));

  } else if (localName == QString::fromAscii ("error")) {

    enter_event (new LogErrorEvent (mp_locator->lineNumber ()));

  } else if (localName == QString::fromAscii ("block")) {

    m_data_stack.push_back (tl::Variant::empty_list ());

  } else if (localName == QString::fromAscii ("string") ||
             localName == QString::fromAscii ("int") ||
             localName == QString::fromAscii ("img")) {
...
{
  if (! m_in_event) {
    // @@@ throw tl::XMLLocatedException (tl::to_string (QObject::tr ("Unexpected element")), ex.lineNumber (), ex.columnNumber ());
    return;
  }
