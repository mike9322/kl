...
public:
  ...
  {
    if (mp_image) {
      db::Matrix3d m = db::Matrix3d::disp (p0 + db::DPoint (nx * dx * 0.5, ny * dy * 0.5)) * db::Matrix3d::mag (dx, dy);
...
class TileOutputReceiver_Impl 
  : public db::TileOutputReceiver, public gsi::ObjectBase
{
public:
  ...
  void set_processor (db::TilingProcessor *proc)
  {
    mp_proc = proc;
  }
  ...
  virtual void begin (size_t nx, size_t ny, const db::DPoint &p0, double dx, double dy) 
  { 
    m_mt_mode = (mp_proc && mp_proc->threads () >= 1);
    m_events.clear ();
    if (begin_cb.can_issue ()) {
      begin_cb.issue<db::TileOutputReceiver, size_t, size_t, const db::DPoint &, double, double> (&db::TileOutputReceiver::begin, nx, ny, p0, dx, dy);
    } else {
      db::TileOutputReceiver::begin (nx, ny, p0, dx, dy);
    }
  }
  ...
  { 
    ...
    if (finish_cb.can_issue ()) {
      finish_cb.issue<db::TileOutputReceiver> (&db::TileOutputReceiver::finish);
    } else {
      db::TileOutputReceiver::finish ();
    }
  }
...
private:
  ...
  db::TilingProcessor *mp_proc;
  bool m_mt_mode;
};

gsi::Class<TileOutputReceiver_Impl> decl_TileOutputReceiver ("TileOutputReceiver", 
  ...
  gsi::callback ("finish", &TileOutputReceiver_Impl::finish, &TileOutputReceiver_Impl::finish_cb,
...
static void tp_output (db::TilingProcessor *proc, const std::string &name, TileOutputReceiver_Impl *rec)
{
  rec->set_processor (proc);
  rec->keep ();
...
{
  proc->output (name, rdb, cell_id, category_id);
}
