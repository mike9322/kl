...
int 
XORToolDialog::exec (lay::LayoutView *view)
{
  mp_view = view;
    ...
    for (std::vector<db::DBox>::const_iterator box = boxes.begin (); box != boxes.end (); ++box) {
      ...
      for (size_t nw = 0; nw < ntiles_w; ++nw) {
        ...
        for (size_t nh = 0; nh < ntiles_h; ++nh) {
          ...
          region_a.enlarge (db::Point (tile_enlargement_a, tile_enlargement_a));
          region_b.enlarge (db::Point (tile_enlargement_b, tile_enlargement_b));
