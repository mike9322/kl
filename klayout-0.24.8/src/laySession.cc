...
Session::restore (lay::MainWindow &mw)
{
  ...
  std::map <std::string, std::string> filenames_by_name;
  for (std::vector <SessionLayoutDescriptor>::const_iterator ld = m_layouts.begin (); ld != m_layouts.end (); ++ld) {
    filenames_by_name.insert (std::make_pair (ld->name, ld->file_path));
  }
  ...
  for (unsigned int iv = 0; iv < m_views.size (); ++iv) {
    ...
    for (std::vector<SessionCellViewDescriptor>::const_iterator cvd = vd.cellviews.begin (); cvd != vd.cellviews.end (); ++cvd) {
      ...
      if (lh) {
        ...
        std::map <std::string, std::string>::const_iterator ld = filenames_by_name.find (cvd->layout_name);

        bool ok = false;
        if (ld != filenames_by_name.end ()) {
          try {
            cv = view->load_layout (ld->second, true /*add*/);
            ok = true;
          } catch (...) { }
        } 
        if (!ok) {
