...
SaveLayoutOptions::SaveLayoutOptions ()
  : m_format ("GDS2"), m_all_layers (true), m_all_cells (true), m_dbu (0.0), m_scale_factor (1.0),
    mp_options (0), m_keep_instances (false), m_write_context_info (true), m_dont_write_empty_cells (false)
{
  // .. nothing yet ..
}
...
SaveLayoutOptions::operator= (const SaveLayoutOptions &d)
{
  if (this != &d) {
    ...
    if (d.get_specific_options ()) {
      set_specific_options (d.get_specific_options ()->clone ());
    } else {
      set_specific_options (0);
    }
...
void 
SaveLayoutOptions::set_specific_options (const FormatSpecificWriterOptions &options)
{
  set_specific_options (options.clone ());
}
...
SaveLayoutOptions::get_specific_options () const
{
  return mp_options;
}
