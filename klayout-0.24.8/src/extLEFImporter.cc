...
LEFImporter::get_iteration (db::Layout &layout)
{
  ...
  for (long i = 0; i < nx; ++i) {
    for (long j = 0; j < ny; ++j) {
      t.push_back (db::Trans (db::Point::from_double (db::DPoint (dx * i / layout.dbu (), dy * j / layout.dbu ()))));
    }
  }
  return t;
}
  ...
  while (true) {
    ...
    if (test ("CLASS")) {
      ...
      while (! peek (";") && ! peek ("DO")) {
        ...
        double x = get_double ();
        double y = get_double ();
        points.push_back (db::Point::from_double (db::DPoint (x / dbu, y / dbu)));
        test (")");
      }
    ...
    if (test ("END")) {
      ...
      if (test ("VIARULE")) {
        ...
        while (! test ("END")) {
          ...
          if (test ("CUTSIZE")) {
            ...
            x = get_double ();
            y = get_double ();
            cutsize = db::Point::from_double (db::DPoint (x / layout.dbu (), y / layout.dbu ()));
            ...
            x = get_double ();
            y = get_double ();
            cutspacing = db::Point::from_double (db::DPoint (x / layout.dbu (), y / layout.dbu ()));
            ...
            x = get_double ();
            y = get_double ();
            offset = db::Point::from_double (db::DPoint (x / layout.dbu (), y / layout.dbu ()));
            ...
            x = get_double ();
            y = get_double ();
            be = db::Point::from_double (db::DPoint (x / layout.dbu (), y / layout.dbu ()));

            x = get_double ();
            y = get_double ();
            te = db::Point::from_double (db::DPoint (x / layout.dbu (), y / layout.dbu ()));
            ...
            x = get_double ();
            y = get_double ();
            bo = db::Point::from_double (db::DPoint (x / layout.dbu (), y / layout.dbu ()));

            x = get_double ();
            y = get_double ();
            to = db::Point::from_double (db::DPoint (x / layout.dbu (), y / layout.dbu ()));
      ...
      while (! at_end ()) {
        ...
        if (test ("END")) {
          ...
          double y = get_double ();
          expect (";");
          origin = db::Point::from_double (db::DPoint (x / layout.dbu (), y / layout.dbu ()));
          ...
          double y = get_double ();
          expect (";");
          size = db::Vector::from_double (db::DVector (x / layout.dbu (), y / layout.dbu ()));
            ...
            if (test ("END")) {
              ...
              */
              ...
              for (std::map <std::string, db::Box>::const_iterator b = bboxes.begin (); b != bboxes.end (); ++b) {
                ...
                if (dl.first) {
                  cell.shapes (dl.second).insert (db::Text (label.c_str (), db::Trans (b->second.center ())));
                }
              }
