 ...
 */
KLAYOUT_DLL void collect_clip_boxes (const db::Layout &layout, db::cell_index_type cell_index, unsigned int layer, std::vector <db::Box> &boxes);
 ...
 */
KLAYOUT_DLL void clip_poly (const db::SimplePolygon &poly, const db::Box &box, std::vector <db::SimplePolygon> &clipped_poly, bool resolve_holes = true);
 ...
 *  @param clipped_poly Where the clip results are stored. The clip results are appended to this vector
 */
KLAYOUT_DLL void clip_poly (const db::Polygon &poly, const db::Box &box, std::vector <db::Polygon> &clipped_poly, bool resolve_holes = true);
