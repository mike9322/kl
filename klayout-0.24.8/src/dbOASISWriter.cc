...
// ---------------------------------------------------------------------------------
//  Function determining whether a property shall be produced as S_GDS_PROPERTY
...
template <class Tag> void create_repetition_by_type (const db::Shape &array_shape, db::Repetition &rep, const Tag &tag) 
{
  ...
  if (array->is_iterated_array (&pts)) {
    ...
    tl_assert (! pts.empty ());
    db::Point po = pts.front ();
    std::vector<db::Point>::iterator pw = pts.begin(); 
    for (std::vector<db::Point>::iterator p = pw + 1; p != pts.end (); ++p) {
      *pw++ = *p - po;
    }
 ...
 */
template <class PC>
struct rep_point_cmp
{
  bool operator () (const std::pair <db::Point, std::pair <db::Coord, int> > &a, const std::pair <db::Point, std::pair <db::Coord, int> > &b)
  {
...
Compressor<Obj>::flush (db::OASISWriter *writer) 
{
  ...
  typedef std::vector <std::pair <db::Point, std::pair <db::Coord, int> > > tmp_rep_vector;
  tmp_rep_vector repetitions;
  std::vector<std::pair<db::Point, db::Repetition> > rep_vector;
  ...
  for (typename std_ext::hash_map <Obj, disp_vector>::iterator n = m_normalized.begin (); n != m_normalized.end (); ++n) {
    ...
    if (m_level < 1 || n->second.size () < 10) {
      ...
      std::sort (n->second.begin (), n->second.end (), point_cmp_x ());
      ...
      for (int xypass = 0; xypass <= 1; ++xypass) {
        ...
        if (xrep) {
          std::sort (displacements.begin (), displacements.end (), point_cmp_x ());
        } else {
          std::sort (displacements.begin (), displacements.end (), point_cmp_y ());
        }
        ...
        for (d = displacements.begin (); d != displacements.end (); ) {
          ...
          if (m_level < 2) {
            ...
            if (dd != displacements.end ()) {
              ...
              dxy = xrep ? db::Point (safe_diff (dd->x (), d->x ()), 0) : db::Point (0, safe_diff (dd->y (), d->y ()));
            ...
            for (unsigned int nn = 0; nn < m_level; ++nn) {
              ...
              db::Point dxy = xrep ? db::Point (safe_diff (dd->x (), d->x ()), 0) : db::Point (0, safe_diff (dd->y (), d->y ()));
            ...
            if (nxy_max < 2) {
              ...
              db::Point dxy_max = xrep ? db::Point (safe_diff ((d + nn_max + 1)->x (), d->x ()), 0) : db::Point (0, safe_diff ((d + nn_max + 1)->y (), d->y ()));

              disp_vector::iterator ds = dwindow;
              disp_vector::iterator dt = dwindow;
              db::Point df = *d + dxy_max * (nxy_max - 1);
        ...
        for (int xypass2 = 1; xypass2 >= 0; --xypass2) {
          ...
          if (xypass2) {
            std::sort (repetitions.begin (), repetitions.end (), rep_point_cmp<point_cmp_y> ());
          } else {
            std::sort (repetitions.begin (), repetitions.end (), rep_point_cmp<point_cmp_x> ());
          }
          ...
          for (tmp_rep_vector::const_iterator r = repetitions.begin (); r != repetitions.end (); ) {
            ...
            if (rr != repetitions.end ()) {
              dxy2 = xypass2 ? db::Point (0, safe_diff (rr->first.y (), r->first.y ())) : db::Point (safe_diff (rr->first.x (), r->first.x ()), 0);
            }
            int nxy2 = 1;
            ...
            if (nxy2 < 2 && xypass2) {
              *rw++ = *r;
              ...
              if (m_level < 2) {
                Obj obj = n->first;
                obj.move (r->first);
                db::Point a (xrep ? r->second.first : 0, xrep ? 0 : r->second.first);
      ...
      if (m_level > 1) {
        ...
        if (! n->second.empty ()) {
          ...
          for (std::vector<db::Point>::const_iterator d = n->second.begin () + 1; d != n->second.end (); ++d) {
            array_cost += std::max(1.0, cost_of (d->x () - d[-1].x ()) + cost_of (d->y () - d[-1].y ())); 
          }
        }

        bool array_set = false;
        db::Point a_ref, b_ref;
        size_t in_ref = 0, im_ref = 0;
        bool ref_set = false;
        db::Coord x_ref = 0, y_ref = 0;

        for (std::vector<std::pair<db::Point, db::Repetition> >::const_iterator r = rep_vector.begin (); r != rep_vector.end (); ++r) {
    ...
    if (n->second.size () > 1) {
      ...
      //  need to normalize?
      db::Point p0 = n->second.front ();
      std::vector<db::Point>::iterator pw = n->second.begin(); 
      for (std::vector<db::Point>::iterator p = pw + 1; p != n->second.end (); ++p) {
        *pw++ = *p - p0;
      }
...
OASISWriter::write_astring (const char *s)
{
  ...
  if (valid) {
    ...
    for (const char *c = s; *c; ++c) {
      if ((*c < 0x20 || *c > 0x7e) && ! m_options.subst_char.empty ()) {
        write_bytes (m_options.subst_char.c_str (), 1);
      } else {
        write_byte (*c);
      }
    }
...
OASISWriter::write_nstring (const char *s)
{
  ...
  if (!l) {
    write_astring (m_options.subst_char.c_str ());
  } else {
    ...
    if (valid) {
      ...
      for (const char *c = s; *c; ++c) {
        if ((*c < 0x21 || *c > 0x7e) && ! m_options.subst_char.empty ()) {
          write_bytes (m_options.subst_char.c_str (), 1);
        } else {
          write_byte (*c);
        }
...
void
OASISWriter::write_gdelta (const db::Point &p, double sf)
{
  db::Coord x = p.x ();
  db::Coord y = p.y ();
...
void 
OASISWriter::write (const db::Layout &layout, tl::OutputStream &stream, const db::SaveLayoutOptions &options) 
{
  ...
  const OASISWriterOptions *oasis_options = dynamic_cast <const OASISWriterOptions *> (options.get_specific_options ());
  if (oasis_options) {
    m_options = *oasis_options;
  } else {
    m_options = OASISWriterOptions ();
  }
  ...
  if (m_options.write_std_properties > 0) {
    ...
    for (std::vector<db::cell_index_type>::const_iterator cell = cells.begin (); cell != cells.end (); ++cell) {
      ...
      if (is_top) {
        write_property_def (s_top_cell_name, tl::Variant (layout.cell_name (*cell)), true);
      }
    }
...
OASISWriter::write (const Repetition &rep)
{
  if (mm_repetition == rep) {
    ...
    const std::vector<db::Point> *iterated = rep.is_iterated ();
    ...
    if (iterated) {
      ...
      //  extract common grid
      db::Coord g = 0;
      for (std::vector<db::Point>::const_iterator p = iterated->begin (); p != iterated->end (); ++p) {
      ...
      db::Point last_point;
      for (std::vector<db::Point>::const_iterator p = iterated->begin (); p != iterated->end (); ++p) {
        db::Point s (safe_scale (m_sf, p->x ()), safe_scale (m_sf, p->y ()));
        db::Point delta = s - last_point;
        last_point = s;
        write_gdelta (db::Point (delta.x () / g, delta.y () / g), 1.0);
      }
...
void 
OASISWriter::write_inst_with_rep (const db::CellInstArray &inst, db::properties_id_type prop_id, const db::Point &disp, const db::Repetition &rep)
{
  db::Point tr = inst.front ().disp () + disp;
...
OASISWriter::write (const db::CellInstArray &inst, db::properties_id_type prop_id, const db::Repetition &rep)
{
  ...
  if (is_reg && (amax > 1 || bmax > 1)) {
    ...
    if (rep != db::Repetition ()) {
      ...
      write_inst_with_rep (inst, prop_id, db::Point (), array_rep);
    }

  } else {
    write_inst_with_rep (inst, prop_id, db::Point (), rep);
  }
}
  ...
  for (db::Cell::const_iterator inst_iterator = mp_cell->begin (); ! inst_iterator.at_end (); ++inst_iterator) {
    ...
    if (cell_set.find (inst_iterator->cell_index ()) != cell_set.end ()) {
      ...
      if (level <= 0) {
        ...
        db::Point disp = inst_array.front ().disp ();
...
void
OASISWriter::write_pointlist (const std::vector<db::Point> &pointlist, bool for_polygons)
{
  ...
  int type = -1;
  int hvlast = -1;
  for (std::vector<db::Point>::const_iterator p = pointlist.begin (); p != pointlist.end (); ++p) {
    int hv = -1;
    if (p->x () == plast.x ()) {
      hv = 1;
    } else if (p->y () == plast.y ()) {
  ...
  if (type == 0 || type == 1) {
    ...
    db::Point plast (0, 0);
    for (std::vector<db::Point>::const_iterator p = pointlist.begin (); p != pointlist.end () - implicit; ++p) {
    ...
    if (m_sf == 1.0) {
      ...
      for (std::vector<db::Point>::const_iterator p = pointlist.begin (); p != pointlist.end (); ++p) {
        db::Point ps (safe_scale (m_sf, p->x ()), safe_scale (m_sf, p->y ()));
...
OASISWriter::write (const db::SimplePolygon &polygon, db::properties_id_type prop_id, const db::Repetition &rep)
{
  ...
  db::Point start = *e;
  m_pointlist.assign (++e, polygon.end_hull ());

  for (std::vector<db::Point>::iterator p = m_pointlist.begin (); p != m_pointlist.end (); ++p) {
    *p -= start;
  }
