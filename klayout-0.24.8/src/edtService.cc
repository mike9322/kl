...
db::DPoint 
Service::snap (db::DPoint p, const db::DPoint &plast, bool connect) const
{
  p = plast + lay::snap_angle (db::DVector (p - plast), connect ? connect_ac () : move_ac ());
  return snap (p);
}
...
Service::copy_selected ()
{
  ...
  if (! need_to_ask_for_copy_mode || mode_dialog.exec (inst_mode)) {
...
{
  m_alt_ac = ac;
  db::DPoint p = snap (pu - m_move_start, db::DPoint (), false /*move*/) + snap (m_move_start);
  if (view ()->is_editable () && m_moving) {
    move_markers (db::DTrans (p) * db::DTrans (m_move_trans.fp_trans ()) * db::DTrans (-snap (m_move_start)));
  }
...
{
  m_alt_ac = ac;
  db::DPoint p = snap (pu - m_move_start, db::DPoint (), false) + snap (m_move_start);
  if (view ()->is_editable () && m_moving) {
    move_markers (db::DTrans (p) * db::DTrans (m_move_trans.fp_trans () * tr) * db::DTrans (-snap (m_move_start)));
  }
  m_alt_ac = lay::AC_Global;
}
...
Service::selection_bbox ()
{
  ...
  for (objects::const_iterator r = m_selection.begin (); r != m_selection.end (); ++r) {
    ...
    if (! r->is_cell_inst ()) {
      ...
      const std::vector<db::CplxTrans> *tv_list = tv.per_cv_and_layer (r->cv_index (), r->layer ());
      if (tv_list != 0) {
        for (std::vector<db::CplxTrans>::const_iterator t = tv_list->begin (); t != tv_list->end (); ++t) {
      ...
      const std::vector<db::CplxTrans> *tv_list = tv.per_cv (r->cv_index ());
      if (tv_list != 0) {
        for (std::vector<db::CplxTrans>::const_iterator t = tv_list->begin (); t != tv_list->end (); ++t) {
...
Service::transform (const db::DCplxTrans &trans, const std::vector<db::DCplxTrans> *p_trv)
{
  ...
  for (std::map <std::pair <db::cell_index_type, std::pair <unsigned int, unsigned int> >, std::vector <size_t> >::iterator sbc = shapes_by_cell.begin (); sbc != shapes_by_cell.end (); ++sbc) {
    ...
    if (cv.is_valid ()) {
      ...
      const std::vector<db::CplxTrans> *tv_list = tv.per_cv_and_layer (sbc->first.second.first, sbc->first.second.second);
      if (tv_list != 0) {

        db::DCplxTrans tt = (*tv_list) [0] * db::CplxTrans (cv->layout ().dbu ()) * cv.context_trans ();
        db::CplxTrans mt_mu (tt.inverted () * trans * tt);
        ...
        for (std::vector <size_t>::iterator si = sbc->second.begin (); si != sbc->second.end (); ++si) {
          ...
          if (p_trv != 0 && *si < p_trv->size ()) {
            db::CplxTrans t_mu (tt.inverted () * (*p_trv) [*si] * tt);
    ...
    if (cv.is_valid ()) {
      ...
      if (tv_list != 0) {
        ...
        for (std::vector <size_t>::iterator ii = ibc->second.begin (); ii != ibc->second.end (); ++ii) {
          ...
          //  mt = transformation in DBU units
          db::ICplxTrans mt;
          db::CplxTrans t (i->trans ());
          if (p_trv != 0 && *ii < p_trv->size ()) {
            db::CplxTrans t_mu (tt.inverted () * (*p_trv) [*ii] * tt);
            mt = db::ICplxTrans (t.inverted () * t_mu * t);
          } else {
            mt = db::ICplxTrans (t.inverted () * mt_mu * t);
          }
...
Service::mouse_click_event (const db::DPoint &p, unsigned int buttons, bool prio)
{
  if (view ()->is_editable () && prio && (buttons & lay::RightButton) != 0 && m_editing) {
    m_alt_ac = ac_from_buttons (buttons);
    do_mouse_transform (p, db::DFTrans::r90);
    m_alt_ac = lay::AC_Global;
    return true;
  } else {
...
Service::click_proximity (const db::DPoint &pos, lay::Editable::SelectionMode mode)
{
  ...
  db::DBox search_box = db::DBox (pos, pos).enlarged (db::DPoint (l, l));
  ...
  if (m_cell_inst_service) {
    ...
    std::set< std::pair<db::CplxTrans, int> > variants = view ()->cv_transform_variants ();
    for (std::set< std::pair<db::CplxTrans, int> >::const_iterator v = variants.begin (); v != variants.end (); ++v) {
...
Service::transient_select (const db::DPoint &pos)
{
  ...
  if (m_cell_inst_service) {
    ...
    if (r != finder.end ()) {
      ...
      db::CplxTrans gt = db::CplxTrans (db::Trans (db::Point::from_double (m_move_trans.disp () * (1.0 / cv->layout ().dbu ()))) * db::Trans (m_move_trans.fp_trans ()));
      gt *= (cv.context_trans () * r->trans ());

      tl_assert (r->is_cell_inst () == m_cell_inst_service);

      db::Instance inst = r->back ().inst_ptr;

      std::vector<db::CplxTrans> tv = mp_view->cv_transform_variants (r->cv_index ());
      if (view ()->is_editable ()) {
  ...
  if (m_move_trans != t) {
    ...
    for (std::vector<lay::ViewObject *>::iterator r = m_markers.begin (); r != m_markers.end (); ++r) {
      ...
      if (marker) {
        ...
        const lay::CellView &cv = view ()->cellview (marker->cv_index ());
        double f = 1.0 / cv->layout ().dbu ();
        db::CplxTrans dt = (db::CplxTrans (f) * db::CplxTrans (t)) * (db::CplxTrans (f) * db::CplxTrans (m_move_trans)).inverted ();
        marker->set (dt * marker->trans ());
