 ...
 */
class KLAYOUT_DLL NoMethodError 
  : public EvalError
{
public:
 ...
 */
class KLAYOUT_DLL ExpressionParserContext
  : public tl::Extractor
{
public:
  /**
 ...
 */
class KLAYOUT_DLL ExpressionNode
{
public:
  /**
   *  @brief Constructor
 ...
 */
class KLAYOUT_DLL EvalFunction
{
public:
  /**
   *  @brief Constructor
...
public:
  ...
  /**
   *  @brief Set a reference layout for the layout-related functionality
   *
   *  Layout related functionality is the unit conversion, cell name to index
   *  resolution and layer index computation.
   *  The layout must be specified before the expression is parsed.
   */
  void set_ref_layout (const db::Layout *layout)
  {
    mp_layout = layout;
  }

  /**
   *  @brief Gets the reference layout
   *
   *  If no layout is set locally, the parent context is looked up for one.
   *  If no context layout is present, 0 is returned.
   */
  const db::Layout *ref_layout () const
  {
    if (mp_layout) {
      return mp_layout;
    } else if (mp_parent) {
      return mp_parent->ref_layout ();
    } else {
      return 0;
    }
  }
