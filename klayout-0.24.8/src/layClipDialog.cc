...
public:
  ...
  virtual void get_menu_entries (std::vector<lay::MenuEntry> &menu_entries) const
  {
    ...
    if (lay::Application::instance ()->is_editable ()) {
      menu_entries.push_back (lay::MenuEntry ("clip_tool::show", "clip_tool", "edit_menu.utils_menu.end", tl::to_string (QObject::tr ("Clip Tool"))));
    }
  }
 
   virtual lay::Plugin *create_plugin (db::Manager *, lay::MainWindow *main_window, lay::LayoutView *view) const
   {
     return new ClipDialog (main_window, view);
   }
};
...
ClipDialog::ClipDialog (lay::MainWindow *main, lay::LayoutView *vw)
  : lay::Browser (main, vw), 
    Ui::ClipDialog ()
{
...
BEGIN_PROTECTED
  ...
  if (rb_box1->isChecked ()) {
    ...
    clip_boxes.push_back (db::Box::from_double (db::DBox (db::DPoint (x1, y1), db::DPoint (x2, y2)) * (1.0 / cv->layout ().dbu ())));
    ...
    clip_boxes.push_back (db::Box::from_double (db::DBox (db::DPoint (x - 0.5 * w, y - 0.5 * h), db::DPoint (x + 0.5 * w, y + 0.5 * h)) * (1.0 / cv->layout ().dbu ())));
    ...
    if (ant_service) {
      ...
      while (! ant.at_end ()) {
        clip_boxes.push_back (db::Box::from_double (db::DBox (ant->p1 (), ant->p2 ()) * (1.0 / cv->layout ().dbu ())));
        ++ant;
      }
    }
