...
static void 
flatten_into (db::Layout *layout, db::cell_index_type cell_index, db::cell_index_type target_cell_index, const db::CplxTrans &t, int levels)
{
  layout->flatten (layout->cell (cell_index), layout->cell (target_cell_index), t, levels);
}

static void 
write_simple (const db::Layout *layout, const std::string &filename)
{
...
static void 
write_options1 (const db::Layout *layout, const std::string &filename, const db::SaveLayoutOptions &options)
{
  db::Writer writer (options);
  tl::OutputStream stream (filename);
  writer.write (*layout, stream);
...
static void 
write_options2 (const db::Layout *layout, const std::string &filename, bool gzip, const db::SaveLayoutOptions &options)
{
...
{
  return new db::Layout (lay::Application::instance ()->is_editable (), &manager);
}

static db::Layout *layout_default_ctor()
{
  return new db::Layout (lay::Application::instance ()->is_editable ());
}
...
Class<db::Layout> decl_Layout ("Layout", 
  gsi::constructor ("new", &layout_ctor_with_manager,
    ...
    "store undo information for example. A layout created with this constructor "
    "inherits the editable mode from the application.\n"
    "\n"
    "This method was introduced in version 0.19.\n"
  ) +
  gsi::constructor ("new", &layout_default_ctor,
    "@brief Creates a layout object\n"
    "A layout created with this constructor "
    "inherits the editable mode from the application.\n"
  ) +
  ...
  gsi::method_ext ("layer_indices", &layer_indices,
    "@brief Returns a list of valid layer indices\n"
    "\n"
