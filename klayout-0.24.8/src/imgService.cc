...
{ 
  ...
  db::DPoint dp (0.5 * image_object.width (), 0.5 * image_object.height ());
...
{ 
  ...
  if (m_mode == mode_normal) {
    ...
    frame_p.push_back (db::Polygon::from_double (image_box_poly));
    ...
    vops.reserve (2);
    vops.push_back (lay::ViewOp (canvas.background_color ().rgb (), lay::ViewOp::Copy, 0, lay::ViewOp::Rect, 1, 1));
    vops.push_back (lay::ViewOp (canvas.foreground_color ().rgb (), lay::ViewOp::Copy, 6, lay::ViewOp::Rect, 1, 2));
    ...
    if (! image->landmarks ().empty ()) {
      ...
      plane_frame = canvas.plane (lay::ViewOp (canvas.foreground_color ().rgb (), lay::ViewOp::Copy, 0, lay::ViewOp::Rect, 1, -1));
      //  plane_fill and plane are prio 3 and 4 to be above the normal selection which is 1 and 2
      std::vector <lay::ViewOp> ops;
      ops.push_back (lay::ViewOp (canvas.background_color ().rgb (), lay::ViewOp::Copy, 0, lay::ViewOp::Rect, 3, 3));
      ops.push_back (lay::ViewOp (canvas.foreground_color ().rgb (), lay::ViewOp::Copy, 0, lay::ViewOp::Rect, 1, 4));
      ...
      for (std::vector <db::DPoint>::const_iterator hb = handles.begin (); hb != handles.end (); ++hb)
      {
        ...
        canvas.renderer ().draw (box.enlarged (db::DPoint (d, d)), db::DCplxTrans (), 0, plane_landmarks, 0, 0);
        canvas.renderer ().draw (db::DEdge (p + db::DPoint (3.0 * d, 0), p - db::DPoint (3.0 * d, 0)), db::DCplxTrans (), 0, plane_landmarks, 0, 0);
        canvas.renderer ().draw (db::DEdge (p + db::DPoint (0, 3.0 * d), p - db::DPoint (0, 3.0 * d)), db::DCplxTrans (), 0, plane_landmarks, 0, 0);
      }
      ...
      plane = canvas.plane (lay::ViewOp (canvas.foreground_color ().rgb (), lay::ViewOp::Copy, 0));
      //  plane_fill is prio 3 to be above the normal selection which is 1 and 2
      plane_fill = canvas.plane (lay::ViewOp (canvas.foreground_color ().rgb (), lay::ViewOp::Copy, 0, lay::ViewOp::Rect, 1, 3));
      ...
      for (std::vector <db::DPoint>::const_iterator hb = handles.begin (); hb != handles.end (); ++hb)
      {
        ...
        db::DPolygon handle_box_poly (box.enlarged (db::DPoint (3 / canvas.resolution (), 3 / canvas.resolution ())));
...
Service::insert_image (const img::Object &image)
{
  ...
  //  send a signal
  m_image_list_changed.signal_observers ();

  //  KLUDGE: the const_cast will allow us to modfiy the object begin the DUserObject - that is not really clean
...
dragging_what (const img::Object *iobj, const db::DBox &search_dbox, img::Service::MoveMode &mode, size_t &landmark, db::DPoint &p1)
{
  ...
  db::DPoint dp (0.5 * iobj->width (), 0.5 * iobj->height ());
...
Service::move (const db::DPoint &p, lay::angle_constraint_type ac)
{
  ...
  if (m_move_mode == move_selected) {
    ...
    if (m_move_mode == move_all) {
      ...
      db::DPoint dx (0.5 * m_current.width (), 0.5 * m_current.height ());
      ...
      db::Matrix3d m = m_current.matrix () * db::Matrix3d::disp (db::DPoint () + v + vv * 0.5) * db::Matrix3d::mag (pw, ph);
...
Service::end_move (const db::DPoint &, lay::angle_constraint_type)
{
  if (! m_selected_image_views.empty () && ! m_selected.empty ()) {
    ...
    if (m_move_mode == move_selected) {
      ...
      //  send a signal that at least one image has changed
      m_image_changed.signal_observers ();

    } else if (m_move_mode == move_landmark) {

      //  replace the image that was moved
      mp_view->annotation_shapes ().replace (m_selected.begin ()->first, db::DUserObject (new img::Object (m_current)));
...
Service::paste ()
{
  ...
  //  send a signal that the image collection has changed  
  m_image_list_changed.signal_observers ();
...
Service::erase_image (obj_iterator pos)
{
  ...
  //  send a signal that the image list has changed
  m_image_list_changed.signal_observers ();
}
...
{
  //  replace the object
  mp_view->annotation_shapes ().replace (pos, db::DUserObject (new img::Object (to)));

  //  and make selection "visible"
  selection_to_view ();
    
  //  send a signal that an image has changed  
  m_image_changed.signal_observers ();
}
...
Service::add_image ()
{
  ...
  AddNewImageDialog dialog (lay::MainWindow::instance (), new_image);
  if (dialog.exec ()) {
