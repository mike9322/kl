  ...
  : public ArrayBase
{
  ...
  typedef db::box <coord_type> box_type;
  typedef db::point <coord_type> point_type;
  typedef db::complex_trans <coord_type, db::DCoord> complex_trans_type;
  ...
  virtual bool is_regular_array (point_type & /*a*/, point_type & /*b*/, unsigned long & /*amax*/, unsigned long & /*bmax*/) const 
  {
    return false;
  }

  virtual bool is_iterated_array (std::vector<point_type> *v)
  {
    return false;
  }
  ...
  void transform (const icomplex_trans_type &ct) 
  { 
    transform (complex_trans_type (ct));
  }
};
...
class KLAYOUT_DLL ArrayRepository
{
public:
  ...
  : public basic_array_iterator <Coord>
{
  ...
  regular_array_iterator (const point_type &a, const point_type &b, unsigned long amin, unsigned long amax, unsigned long bmin, unsigned long bmax)
    : m_a (a), m_b (b),
  ...
  {
    return disp_type (point_type (m_ai * m_a.x () + m_bi * m_b.x (), m_ai * m_a.y () + m_bi * m_b.y ()));
  }
  ...
  : public basic_array <Coord>
{
  ...
  regular_array (const point_type &a, const point_type &b, unsigned long amax, unsigned long bmax)
  ...
  virtual box_type bbox (const box_type &obox) const
  {
    if (obox.empty ()) {
      ...
      point_type ma (m_a.x () * (m_amax - 1), m_a.y () * (m_amax - 1));
      point_type mb (m_b.x () * (m_bmax - 1), m_b.y () * (m_bmax - 1));
      ...
      return box_type (obox.p1 () + b.p1 (), obox.p2 () + b.p2 ());
  ...
  { 
    ...
    m_a = point_type::from_double (ct * m_a);
    m_a -= point_type::from_double (ct.disp ());
    m_b = point_type::from_double (ct * m_b);
    m_b -= point_type::from_double (ct.disp ());
    compute_det ();
  }
  ...
  : public regular_array<Coord>
{
  ...
  regular_complex_array (double acos, double mag, const point_type &a, const point_type &b, unsigned long amax, unsigned long bmax)
  ...
  virtual void invert (simple_trans_type &t) 
  {
    ...
    //  is inverted. 
    complex_trans_type r = complex_trans_type (t, m_acos, m_mag);
    r.invert ();
    m_mag = r.mag ();
    m_acos = r.rcos ();
    t = simple_trans_type (r);
    regular_array<Coord>::m_a = -point_type::from_double (r (regular_array<Coord>::m_a) - r (point_type ()));
    regular_array<Coord>::m_b = -point_type::from_double (r (regular_array<Coord>::m_b) - r (point_type ()));
...
struct iterated_array_iterator
  : public basic_array_iterator <Coord>
{
  typedef typename basic_array_iterator<Coord>::point_type coord_type;
  typedef typename basic_array_iterator<Coord>::point_type point_type;
  typedef typename basic_array_iterator<Coord>::box_type box_type;
  typedef typename basic_array_iterator<Coord>::disp_type disp_type;
  typedef box_convert<point_type> box_convert_type;
  typedef unstable_box_tree <box_type, point_type, box_convert_type> box_tree_type;
  ...
  : public basic_array <Coord>
{
  ...
  void sort ()
  {
    m_v.sort (db::box_convert <point_type> ());
  }
  ...
  virtual box_type bbox (const box_type &obox) const
  {
    if (obox.empty ()) {
      ...
      return box_type (obox.p1 () + m_box.p1 (), obox.p2 () + m_box.p2 ());
    }
  }
  ...
  { 
    m_box = box_type ();
    db::Point d = point_type::from_double (ct.disp ());
    for (iterator p = m_v.begin (); p != m_v.end (); ++p) {
      *p = point_type::from_double (ct * *p) - d;
      m_box += *p;
    }
    sort ();
  }
  ...
  : public iterated_array<Coord>
{
  ...
  virtual void invert (simple_trans_type &t) 
  {
    ...
    for (typename iterated_array<Coord>::iterator p = this->m_v.begin (); p != this->m_v.end (); ++p) {
      *p = -point_type::from_double (r (*p) - r (point_type ()));
      this->m_box += *p;
    }
...
struct array
{
   ...
   */
  array (const Obj &obj, const trans_type &trans, const point_type &a, const point_type &b, unsigned long amax, unsigned long bmax)
   ...
   *  it's own storage.
   */
  array (const Obj &obj, const trans_type &trans, ArrayRepository &rep, const point_type &a, const point_type &b, unsigned long amax, unsigned long bmax)
   ...
   *  an appropriate basic_array object.
   */
  array (const Obj &obj, const trans_type &trans, double acos, double mag, const point_type &a, const point_type &b, unsigned long amax, unsigned long bmax)
   ...
   */
  array (const Obj &obj, const complex_trans_type &ct, const point_type &a, const point_type &b, unsigned long amax, unsigned long bmax)
   ...
   *  it's own storage.
   */
  array (const Obj &obj, const trans_type &trans, ArrayRepository &rep, double acos, double mag, const point_type &a, const point_type &b, unsigned long amax, unsigned long bmax)
   ...
   *  it's own storage.
   */
  array (const Obj &obj, const complex_trans_type &ct, ArrayRepository &rep, const point_type &a, const point_type &b, unsigned long amax, unsigned long bmax)
    ...
    if (b.empty ()) {
      ...
      if (ob.empty ()) {
        ...
        if (mp_base->is_complex ()) {
          complex_trans_type ct = mp_base->complex_trans (m_trans);
          ct.disp (typename complex_trans_type::displacement_type ());
          ob = box_type::from_double (ct * ob);
        } else {
          ob.transform (db::fixpoint_trans<coord_type> (m_trans.rot ()));
        }
        point_type d = m_trans (point_type ());
        return array_iterator <coord_type, Trans> (m_trans, mp_base->begin_touching (box_type (b.p1 () - ob.p2 () - d, b.p2 () - ob.p1 () - d)));
      }
    } else {
      box_type ob (bc (m_obj));
      if (ob.empty ()) {
        ...
        point_type d = m_trans (point_type ());
        ob.transform (db::fixpoint_trans<coord_type> (m_trans.rot ()));
        return array_iterator <coord_type, Trans> (m_trans, ! box_type (b.p1 () - ob.p2 (), b.p2 () - ob.p1 ()).contains (d));
      }
    }
  }
  ...
  box_type bbox (const BoxConv &bc) const
  {
    if (mp_base) {
      if (mp_base->is_complex ()) {
        return mp_base->bbox (box_type::from_double (mp_base->complex_trans (m_trans) * bc (m_obj)));
      } else {
  ...
  box_type bbox_from_raw_bbox (const box_type &rb, const BoxConv &bc) const
  {
    if (mp_base) {
      if (mp_base->is_complex ()) {
        return rb * (box_type::from_double (mp_base->complex_trans (m_trans.fp_trans ()) * bc (m_obj)));
      } else {
  ...
  {
    return mp_base ? mp_base->complex_trans (m_trans) : complex_trans_type (m_trans);
  }
  ...
  {
    return mp_base ? mp_base->complex_trans (trans) : complex_trans_type (trans);
  }
   ...
   *  one. 
   */
  bool is_regular_array (point_type &a, point_type &b, unsigned long &amax, unsigned long &bmax) const 
  {
   ...
   */
  bool is_iterated_array (std::vector<point_type> *v = 0) const
  {
   ...
   */
  void move (const point_type &pt, db::ArrayRepository *array_rep = 0)
  {
...
private:
  ...
  {
    if (is_complex ()) {
      complex_trans_type t = complex_trans_type (tr) * d.complex_trans () * complex_trans_type (tr).inverted ();
      m_trans = Trans (t);
  ...
  void transform_into_from (const icomplex_trans_type &tr, const array<Obj, Trans> &d)
  {
    complex_trans_type t = complex_trans_type (tr) * d.complex_trans () * complex_trans_type (tr).inverted ();
    m_trans = Trans (t);
    set_complex (t.mag (), t.rcos (), d);
  ...
  {
    complex_trans_type t = tr * d.complex_trans () * tr.inverted ();
    m_trans = Trans (t);
    set_complex (t.mag (), t.rcos (), d);
  }
  ...
  void translate_from (const disp_trans_type &tr, const array<Obj, Trans> &d, db::generic_repository<coord_type> &rep, db::ArrayRepository &array_rep)
  {
    if (is_complex ()) {
      ...
      m_trans = Trans (t);
      set_complex (t.mag (), t.rcos (), d);
      m_obj.translate (d.m_obj, icomplex_trans_type (complex_trans ().inverted () * complex_trans_type (t)), rep, array_rep);
    } else {
  ...
  void translate_from (const icomplex_trans_type &tr, const array<Obj, Trans> &d, db::generic_repository<coord_type> &rep, db::ArrayRepository &array_rep)
  {
    if (is_complex ()) {
      complex_trans_type t = complex_trans_type (tr) * d.complex_trans ();
      m_trans = Trans (t);
      ...
      icomplex_trans_type t = tr * icomplex_trans_type (d.front ());
      m_trans = Trans (t);
      m_obj.translate (d.m_obj, icomplex_trans_type (m_trans.inverted ()) * t, rep, array_rep);
    }
  }
  ...
  void translate_from (const complex_trans_type &tr, const array<Obj, Trans> &d, db::generic_repository<coord_type> &rep, db::ArrayRepository &array_rep)
  {
    if (is_complex ()) {
      ...
      m_trans = Trans (t);
      m_obj.translate (d.m_obj, icomplex_trans_type (complex_trans_type (m_trans.inverted ()) * t), rep, array_rep);
    }
  }
  ...
  {
    m_trans = Trans (simple_trans_type (tr) * simple_trans_type (d.front ()));
  }

  void transform_from (const simple_trans_type &tr, const array<Obj, Trans> &d)
  {
    m_trans = Trans (tr * simple_trans_type (d.front ()));
  }

  void transform_from (const icomplex_trans_type &tr, const array<Obj, Trans> &d)
  {
    transform_from (complex_trans_type (tr), d);
  }
...
/**
 *  @brief A standard typedef for a box array
 */
typedef db::array <db::Box, db::Trans> BoxArray;

/**
 *  @brief A standard typedef for a box array in double coordinates
 */
typedef db::array <db::DBox, db::Trans> DBoxArray;
