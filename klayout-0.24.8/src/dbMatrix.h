...
public:
  ...
  Matrix2d (const Tr &t)
  {
    db::vector<typename Tr::target_coord_type> v1 = t * db::vector<typename Tr::coord_type> (1, 0);
    db::vector<typename Tr::target_coord_type> v2 = t * db::vector<typename Tr::coord_type> (0, 1);
...
public:
  ...
  Matrix3d (const Tr &t)
  {
    db::point<typename Tr::target_coord_type> v0 = t * db::point<typename Tr::coord_type> (0, 0);
    db::vector<typename Tr::target_coord_type> v1 = t * db::vector<typename Tr::coord_type> (1, 0);
    db::vector<typename Tr::target_coord_type> v2 = t * db::vector<typename Tr::coord_type> (0, 1);
    set (v1.x (), v2.x (), v0.x (), v1.y (), v2.y (), v0.y (), 0.0, 0.0, 1.0);
  }
   ...
   */
  db::DPoint disp () const;

  /**
   *  @brief Create the mirror matrix
   */
  static Matrix3d disp (const db::DPoint &d)
  {
 ...
 */
void KLAYOUT_DLL adjust_matrix (Matrix2d &matrix, db::DPoint &disp, const std::vector <db::DPoint> &landmarks_before, const std::vector <db::DPoint> &landmarks_after, MatrixAdjustFlags::Flags flags, int fixed_point = -1);
 ...
 *  @param fixed_point The index of the fixed point (one that is definitly mapped to the target) or -1 if there is none
 */
void KLAYOUT_DLL adjust_matrix (Matrix3d &matrix, const std::vector <db::DPoint> &landmarks_before, const std::vector <db::DPoint> &landmarks_after, MatrixAdjustFlags::Flags flags, int fixed_point = -1);
...
namespace tl 
{
  template<> KLAYOUT_DLL void extractor_impl (tl::Extractor &ex, db::Matrix2d &t);
  template<> KLAYOUT_DLL void extractor_impl (tl::Extractor &ex, db::Matrix3d &t);
  template<> KLAYOUT_DLL bool test_extractor_impl (tl::Extractor &ex, db::Matrix2d &t);
  template<> KLAYOUT_DLL bool test_extractor_impl (tl::Extractor &ex, db::Matrix3d &t);
} // namespace tl
