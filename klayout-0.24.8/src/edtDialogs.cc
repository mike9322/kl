...
{
  setObjectName (QString::fromAscii ("instantiation_form"));
...
CopyModeDialog::CopyModeDialog (QWidget *parent)
  : QDialog (parent)
{
  setObjectName (QString::fromAscii ("copy_mode_dialog"));
...
bool 
CopyModeDialog::exec (unsigned int &mode)
{
  if (mode == 0) {
...
ChangeLayerOptionsDialog::ChangeLayerOptionsDialog (QWidget *parent)
  : QDialog (parent)
{
  setObjectName (QString::fromAscii ("change_layer_options_dialog"));
...
bool 
ChangeLayerOptionsDialog::exec (lay::LayoutView *view, int cv_index, unsigned int &new_layer)
{
...
bool 
AlignOptionsDialog::exec (lay::LayoutView *view, int &hmode, int &vmode, bool &visible_layers)
{
...
bool 
MakeCellOptionsDialog::exec (const db::Layout &layout, std::string &name) 
{
  do {
BEGIN_PROTECTED
    if (QDialog::exec ()) {
...
{
  setObjectName (QString::fromAscii ("round_corners_options_dialog"));
...
bool 
RoundCornerOptionsDialog::exec (const db::Layout &layout, double &router, double &rinner, unsigned int &npoints)
{
  mp_layout = &layout;
...
bool 
MakeArrayOptionsDialog::exec (db::DPoint &a, unsigned int &na, db::DPoint &b, unsigned int &nb)
{
