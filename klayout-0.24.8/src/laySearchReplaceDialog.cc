...
static std::string instance_to_string (const db::Instance &inst, const db::CplxTrans &t = db::CplxTrans ())
{
  ...
  if (inst.instances () && inst.instances ()->cell () && inst.instances ()->cell ()->layout ()) {
    r = inst.instances ()->cell ()->layout ()->cell_name (ci.object ().cell_index ());
...
static std::string shape_to_string (const db::Shape &shape, const db::CplxTrans &t = db::CplxTrans ())
{
  double dbu = 1.0;
...
SearchReplaceResults::export_rdb (rdb::Database &rdb, double dbu)
{
  if (! m_cell_result.empty ()) {
    ...
    for (std::vector<QueryInstResult>::const_iterator i = m_inst_result.begin (); i != m_inst_result.end (); ++i) {
      ...
      std::map<std::pair<db::cell_index_type, db::CplxTrans>, rdb::Cell *>::const_iterator v = cells_by_variant.find (std::make_pair (i->cell_index, i->trans));
      if (v == cells_by_variant.end ()) {
        std::map<db::cell_index_type, std::string>::const_iterator cn = m_cellname_map.find (i->cell_index);
        if (cn != m_cellname_map.end ()) {
          v = cells_by_variant.insert (std::make_pair (std::make_pair (i->cell_index, i->trans), rdb.create_cell (cn->second))).first;
          v->second->references ().insert (rdb::Reference (db::CplxTrans (dbu) * i->trans * db::CplxTrans (1.0 / dbu), rdb_top_cell->id ()));
        }
      }
    ...
    for (std::vector<QueryShapeResult>::const_iterator s = m_shape_result.begin (); s != m_shape_result.end (); ++s) {
      ...
      if (cm != categories.end ()) {
        ...
        std::map<std::pair<db::cell_index_type, db::CplxTrans>, rdb::Cell *>::const_iterator v = cells_by_variant.find (std::make_pair (s->cell_index, s->trans));
        if (v == cells_by_variant.end ()) {
          std::map<db::cell_index_type, std::string>::const_iterator cn = m_cellname_map.find (s->cell_index);
          if (cn != m_cellname_map.end ()) {
            v = cells_by_variant.insert (std::make_pair (std::make_pair (s->cell_index, s->trans), rdb.create_cell (cn->second))).first;
            v->second->references ().insert (rdb::Reference (db::CplxTrans (dbu) * s->trans * db::CplxTrans (1.0 / dbu), rdb_top_cell->id ()));
          }
        }
...
SearchReplaceDialog::SearchReplaceDialog (lay::MainWindow *main, lay::LayoutView *view)
  : lay::Browser (main, view),
    ...
    m_max_item_count (0)
{
  setObjectName (QString::fromAscii ("search_replace_dialog"));
...
{
  lay::PluginRoot *config_root = main_window ();
...
{
  detach_all ();  //  don't listen to layout events any longer
...
void
SearchReplaceDialog::signal_observed ()
{
...
{
  layout->add_hier_observer (*this);
  layout->add_bboxes_observer (*this);
  layout->add_cell_name_observer (*this);
  layout->add_layer_properties_observer (*this);
}
    ...
    if (data_prop_id >= 0 && iq.get (data_prop_id, v)) {
      ...
      if (path_trans_prop_id >= 0 && iq.get (path_trans_prop_id, v)) {
        ...
        if (trans_prop_id >= 0 && iq.get (trans_prop_id, v)) {
          ...
          trans = trans * v.to_user<db::CplxTrans> ().inverted ();
        }
        if (initial_cell_index_prop_id >= 0 && iq.get (initial_cell_index_prop_id, v)) {
          initial_cell_index = db::cell_index_type (v.to_int ());
        }
...
SearchReplaceDialog::result_selection_changed ()
{
  try {
    ...
    std::vector<db::CplxTrans> global_trans = view ()->cv_transform_variants (cv_index);
    std::map<unsigned int, std::vector<db::CplxTrans> > tv_map = view ()->cv_transform_variants_by_layer (cv_index);
    ...
    for (QModelIndexList::const_iterator s = sel.begin (); s != sel.end (); ++s) {
      ...
      if (index < 0) {
        ...
        if (! sr.shape.is_null ()) {
          ...
          std::map<unsigned int, std::vector<db::CplxTrans> >::const_iterator tv = tv_map.find (sr.layer_index);
          if (tv != tv_map.end ()) {
        ...
        std::pair<bool, db::CplxTrans> si = db::find_layout_context (layout, ir.cell_index, cv.cell_index ());
        if (si.first) {
    ...
    if (! dbox.empty ()) {
      ...
      if (m_window == FitCell) {
        ...
        view ()->zoom_box (dbox.enlarged (db::DPoint (m_window_dim, m_window_dim)));
      } else if (m_window == Center) {
        view ()->pan_center ((dbox.p1 () + dbox.p2 ()) * 0.5);
      } else if (m_window == CenterSize) {
        double w = std::max (dbox.width (), m_window_dim);
        double h = std::max (dbox.height (), m_window_dim);
        db::DPoint center ((dbox.p1 () + dbox.p2 ()) * 0.5);
...
{
  lay::ConfigurationDialog config_dialog (this, main_window (), "SearchReplacePlugin");
  config_dialog.exec ();
}
