...
{
  const db::GDS2ReaderOptions *specific_options = dynamic_cast<const db::GDS2ReaderOptions *> (options.get_specific_options ("GDS2"));
  if (specific_options) {
    m_options = *specific_options;
  }
  ...
  return basic_read (layout, options, m_options.allow_multi_xy_records, m_options.box_mode);
}
