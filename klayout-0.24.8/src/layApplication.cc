...
addr2symname (DWORD64 addr)
{
  ...
  if (! mod_name.isNull ()) {
    mod_name = QString::fromAscii ("(") + mod_name + QString::fromAscii (") ");
  }

  free (symbol);

  return QString::fromAscii ("0x%1 - %2%3+%4").
            arg (addr, 0, 16).
            arg (mod_name).
            arg (sym_name).
            arg (disp);
...
LONG WINAPI ExceptionHandler(PEXCEPTION_POINTERS pExceptionInfo)
{
  ...
  text += QObject::tr ("Program Version: ") + 
          QString::fromAscii (lay::Version::name ()) + 
          QString::fromAscii (" ") +
          QString::fromAscii (lay::Version::version ()) +
          QString::fromAscii (" (") +
          QString::fromAscii (lay::Version::subversion ()) +
          QString::fromAscii (")");
#if defined(_WIN64)
  text += QString::fromAscii (" AMD64");
#else
  text += QString::fromAscii (" x86");
#endif
...
void signal_handler (int signo, siginfo_t *si, void *)
{
  ...
  if (symbols == NULL) {
    ...
    for (size_t i = 2; i < nptrs; i++) {
      text += QString::fromAscii (symbols [i]) + QString::fromAscii ("\n");
    }
  }
  free(symbols);
...
get_appdata_path ()
{
...
#ifdef _WIN32
  appdata_folder = QString::fromAscii ("KLayout");
#else
  appdata_folder = QString::fromAscii (".klayout");
#endif
  ...
  for (size_t i = 0; i < sizeof (folders) / sizeof (folders [0]); ++i) {
    QString folder = QString::fromAscii (folders [i]);
...
get_other_system_paths (std::vector <std::string> &p)
{
  ...
  if ((appdata_env = _wgetenv (L"APPDATA")) != 0) {
    ...
    QString appdata_folder = QString::fromAscii ("KLayout");
    ...
    mp_recorder (0)
{
  ...
  m_appdata_path = get_appdata_path ();

  //  get the installation path
  m_inst_path = tl::to_string (QCoreApplication::applicationDirPath ());

  //  generate the klayout path: the first component is always the appdata path
  m_klayout_path.push_back (m_appdata_path);
#ifdef _WIN32
  wchar_t *klayout_path = _wgetenv (L"KLAYOUT_PATH");
  if (klayout_path) {
    split_path (tl::to_string (QString ((const QChar *) klayout_path)), m_klayout_path);
  } else {
    get_other_system_paths (m_klayout_path);
    m_klayout_path.push_back (m_inst_path);
  }
#else
  char *klayout_path = getenv ("KLAYOUT_PATH");
  if (klayout_path) {
    split_path (tl::system_to_string (klayout_path), m_klayout_path);
  } else {
    get_other_system_paths (m_klayout_path);
    m_klayout_path.push_back (m_inst_path);
  }
  ...
  if (mp_qapp_gui) {
    ...
    //  (the ones used for reset) into m_initial_config_files.
    {
      ...
      QDir qd (QDir::home ());
      QString filename = QString::fromAscii (".layviewrc");
    ...
    m_config_file_to_write = tl::to_string (QDir (tl::to_qstring (m_appdata_path)).absoluteFilePath (QString::fromAscii ("klayoutrc")));
    ...
    for (std::vector <std::string>::const_iterator p = m_klayout_path.end (); p != m_klayout_path.begin (); ) {
      ...
      QString filename = QString::fromAscii ("klayoutrc");
      if (qd.exists (filename) && QFileInfo (qd.absoluteFilePath (filename)).isReadable ()) {
        m_config_files.push_back (tl::to_string (qd.absoluteFilePath (filename)));
  ...
  for (std::vector <std::string>::const_iterator p = m_klayout_path.begin (); p != m_klayout_path.end (); ++p) {
    ...
    QFileInfo rbainit_file (tl::to_qstring (*p), QString::fromAscii ("rbainit"));
    ...
    name_filters << QString::fromAscii ("*.rbm");
    name_filters << QString::fromAscii ("*.pym");
  ...
  for (std::vector <std::string>::const_iterator p = m_klayout_path.begin (); p != m_klayout_path.end (); ++p) {
    ...
    QStringList name_filters;
    name_filters << QString::fromAscii ("*.klp");
  ...
  for (std::vector <std::string>::const_iterator p = m_klayout_path.begin (); p != m_klayout_path.end (); ++p) {
    ...
    QStringList name_filters;
    name_filters << QString::fromAscii ("*.lyt");
  ...
  if (mp_qapp_gui) {
    mp_qapp_gui->setWindowIcon (QIcon (QString::fromAscii (":/logo.png")));
  ...
  for (std::vector <std::string>::const_iterator p = m_klayout_path.begin (); p != m_klayout_path.end (); ++p) {
    ...
    for (QStringList::const_iterator im = libs.begin (); im != libs.end (); ++im) {
      ...
      try {
        ...
        for (db::Reader::meta_info_iterator m = reader.begin_meta (); m != reader.end_meta (); ++m) {
