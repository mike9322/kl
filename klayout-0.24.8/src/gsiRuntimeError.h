...
#ifndef _HDR_gsiRuntimeError
#define _HDR_gsiRuntimeError
 ...
 */
struct KLAYOUT_DLL BacktraceElement
{
  /**  
 ...
 *
 *  TODO: this class should go into gsi namespace
 */
class KLAYOUT_DLL RuntimeError
  : public tl::Exception 
{
public:
  RuntimeError (const char *msg, const char *cls, const std::vector <BacktraceElement> &backtrace)
    : tl::Exception (msg), m_line (-1), m_cls (cls), m_backtrace (backtrace)
  { }

  RuntimeError (const char *msg, const char *sourcefile, int line, const char *cls, const std::vector <BacktraceElement> &backtrace)
    : tl::Exception (msg), m_sourcefile (sourcefile), m_line (line), m_cls (cls), m_backtrace (backtrace)
  { }

  RuntimeError (const RuntimeError &d)
 ...
 *  on the user side.
 */
class KLAYOUT_DLL ExitException
  : public tl::Exception
{
public:
